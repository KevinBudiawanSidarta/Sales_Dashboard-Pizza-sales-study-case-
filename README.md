...
| Tool | Purpose |
|---|---|
| **Microsoft Power BI Desktop** | Dashboard development, DAX measures, visualizations |
| **SQL Server / SQL Database** | Source data storage and extraction |
| **Power Query (M Language)** | Data transformation and column derivation |
...
2. Open `Pizza_Sales_Dashboard.pbix` in **Microsoft Power BI Desktop** (version 2.0 or later recommended).
3. If connecting to a live SQL database, update the data source credentials under **Home → Transform Data → Data Source Settings**.
4. Refresh the data model to load the latest records.
5. Interact with the dashboard using the slicers and cross-filter functionality built into each visual.

---

*Dashboard covers the period: **January 2015 – December 2015***
